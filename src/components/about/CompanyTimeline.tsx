          ...
          return (
            ...
            >
              <div className="relative flex items-start gap-6 md:gap-0">
                <div className="absolute left-4 md:left-1/2 w-3 h-3 rounded-full bg-brand-pink -translate-x-1.5 md:-translate-x-1.5 mt-1.5 z-10" />
  ...
  return (
    <div
      className={`bg-white rounded-2xl shadow-md p-5 hover:shadow-lg transition-shadow ${
        event.highlight ? "border-l-4 border-brand-pink" : ""
      }`}
    >
      <span className="inline-block text-sm font-bold text-brand-pink mb-1">
        {event.year}
      </span>
      <h3 className="font-bold text-brand-black mb-2">{event.title}</h3>
